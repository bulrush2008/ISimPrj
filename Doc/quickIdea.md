...
49, 数据的随机分割方式，可能不合理。现在的数据是非常有限的，随机分割可能导致训练数据包含的某些流场特征不足
...
54, GAN 的训练，最后两次训练，可以适用“相同”的输入，分别训练 G & D.
  2025.10.22, 13:35

55, 可能是现在的数据有问题，除了尝试新的归一化分布外，还可以使用的新的数据
  - 验证现在 GAN 结构是否有问题
  2025.11.13, 17:21

56, GAN 中，可以改变训练次序 1:2 的关系，可能对于收敛性有帮助
  2025.11.13, 18:07